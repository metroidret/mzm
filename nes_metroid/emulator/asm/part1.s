    .include "asm/constants.inc"
    .include "asm/macros.inc"
	.include "asm/emulator_constants.inc"

    .syntax unified

	@ input r6=0x06007C00 (dst?)
	arm_func_start sub_06006000
sub_06006000: @ 0x06006000
	@ Creates a u32 table that is 256 entries long at r6
	@ It goes through all values 0x00 to 0xFF (stored in r7)
	@ From MSB to LSB, if bit X is set, then set the first bit of the (7-X)th nybble
	@ Some kind of 1bpp to 4bpp LUT?
	mov r7, #0xff
	add r6, r6, r7, lsl #2
_06006008:
	lsr r8, r7, #7
	tst r7, #0x40
	orrne r8, r8, #0x10
	tst r7, #0x20
	orrne r8, r8, #0x100
	tst r7, #0x10
	orrne r8, r8, #0x1000
	tst r7, #8
	orrne r8, r8, #0x10000
	tst r7, #4
	orrne r8, r8, #0x100000
	tst r7, #2
	orrne r8, r8, #0x1000000
	tst r7, #1
	orrne r8, r8, #0x10000000
	str r8, [r6], #-0x4
	subs r7, r7, #1
	bhs _06006008

	@ Store 0x01820182 at 0x06003000-0x0600303F and 0x06003800-0x0600383F
	mov r6, #0x100
	orr r6, r6, #0x82
	orr r6, r6, r6, lsl #16 @ r6 = 0x01820182
	mov r7, #0x3000
	orr r7, r7, #0x6000000  @ r7 = 0x06003000
_06006064:
	str r6, [r7, #0x800]
	str r6, [r7], #4
	tst r7, #0x3c
	bne _06006064

	mov r7, #0x6700
	orr r7, r7, #0x6000000
	strh r6, [r7] @ 0x06006700 = 0x0182

	@ Copy 0x120 bytes of _06006110 using the 0x06007C00 LUT to 0x0600C000
	@ This seems to be 0-9 and uppercase alphabet
	mov r5, #0x7c00
	orr r5, r5, #0x6000000 @ r5 = 0x06007C00
	mov r4, #0xc000
	orr r4, r4, #0x6000000 @ r4 = 0x0600C000
	add r3, pc, #0x78 @ =_06006110
	add r2, r3, #0x120
_06006098:
	ldrb r0, [r3], #1
	ldr r0, [r5, r0, lsl #2]
	str r0, [r4], #4
	teq r3, r2
	bne _06006098

	@ Copy 0xE0 bytes of _06006230 using the 0x06007C00 LUT to 0x0600C480
	@ Each byte is ORed with itself left shifted by 1 (possibly for thickness?)
	@ This seems to be lowercase alphabet tiles, as well as ? and -
	add r2, r3, #0xe0
_060060B0:
	ldrb r0, [r3], #1
	ldr r0, [r5, r0, lsl #2]
	orr r0, r0, r0, lsl #1
	str r0, [r4], #4
	teq r3, r2
	bne _060060B0

	@ Clear 0x0600DFE0-0x0600DFFF
	mov r2, #0xe000
	orr r2, r2, #0x6000000 @ r2 = 0x0600E000
	sub r3, r2, #0x20
	mov r0, #0
_060060D8:
	str r0, [r3], #4
	teq r3, r2
	bne _060060D8

	@ Copy 0x50 bytes of _06006110 using the 0x06007C00 LUT to 0x06011400
	@ Each byte is shifted by 1 (why?)
	@ This seems to be 0-9 tiles
	ldr r4, _0600610C @ =0x06011400
	add r3, pc, #0x20 @ =_06006110
	add r2, r3, #0x50
_060060F0:
	ldrb r0, [r3], #1
	ldr r0, [r5, r0, lsl #2]
	lsl r0, r0, #1
	str r0, [r4], #4
	teq r3, r2
	bne _060060F0

	b sub_0600E1C4 @ goto sub_0600E1C4() (no return)
	.align 2, 0
_0600610C: .4byte 0x06011400
_06006110:
	.byte 0x38, 0x4C, 0xCE, 0xC6, 0xE6, 0x64, 0x38, 0x00, 0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00
	.byte 0x7C, 0xC6, 0x0E, 0x3C, 0x78, 0xE0, 0xFE, 0x00, 0x7E, 0x0C, 0x18, 0x3C, 0x06, 0xC6, 0x7C, 0x00
	.byte 0x1C, 0x3C, 0x6C, 0xCC, 0xFE, 0x0C, 0x0C, 0x00, 0xFC, 0xC0, 0xFC, 0x06, 0x06, 0xC6, 0x7C, 0x00
	.byte 0x3C, 0x60, 0xC0, 0xFC, 0xC6, 0xC6, 0x7C, 0x00, 0xFE, 0xC6, 0x0C, 0x18, 0x30, 0x30, 0x30, 0x00
	.byte 0x7C, 0xC6, 0xC6, 0x7C, 0xC6, 0xC6, 0x7C, 0x00, 0x7C, 0xC6, 0xC6, 0x7E, 0x06, 0x0C, 0x78, 0x00
	.byte 0x38, 0x6C, 0xC6, 0xC6, 0xFE, 0xC6, 0xC6, 0x00, 0xFC, 0xC6, 0xC6, 0xFC, 0xC6, 0xC6, 0xFC, 0x00
	.byte 0x3C, 0x66, 0xC0, 0xC0, 0xC0, 0x66, 0x3C, 0x00, 0xF8, 0xCC, 0xC6, 0xC6, 0xC6, 0xCC, 0xF8, 0x00
	.byte 0xFE, 0xC0, 0xC0, 0xFC, 0xC0, 0xC0, 0xFE, 0x00, 0xFE, 0xC0, 0xC0, 0xFC, 0xC0, 0xC0, 0xC0, 0x00
	.byte 0x3E, 0x60, 0xC0, 0xCE, 0xC6, 0x66, 0x3E, 0x00, 0xC6, 0xC6, 0xC6, 0xFE, 0xC6, 0xC6, 0xC6, 0x00
	.byte 0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00, 0x1E, 0x06, 0x06, 0x06, 0xC6, 0xC6, 0x7C, 0x00
	.byte 0xC6, 0xCC, 0xD8, 0xF0, 0xF8, 0xDC, 0xCE, 0x00, 0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x7E, 0x00
	.byte 0xC6, 0xEE, 0xFE, 0xFE, 0xD6, 0xC6, 0xC6, 0x00, 0xC6, 0xE6, 0xF6, 0xFE, 0xDE, 0xCE, 0xC6, 0x00
	.byte 0x7C, 0xC6, 0xC6, 0xD6, 0xC6, 0xC6, 0x7C, 0x00, 0xFC, 0xC6, 0xC6, 0xC6, 0xFC, 0xC0, 0xC0, 0x00
	.byte 0x7C, 0xC6, 0xC6, 0xC6, 0xDE, 0xCC, 0x7A, 0x00, 0xFC, 0xC6, 0xC6, 0xCE, 0xF8, 0xDC, 0xCE, 0x00
	.byte 0x78, 0xCC, 0xC0, 0x7C, 0x06, 0xC6, 0x7C, 0x00, 0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00
	.byte 0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0x7C, 0x00, 0xC6, 0xC6, 0xC6, 0xEE, 0x7C, 0x38, 0x10, 0x00
	.byte 0xC6, 0xC6, 0xD6, 0xFE, 0xFE, 0xEE, 0xC6, 0x00, 0xC6, 0xEE, 0x7C, 0x38, 0x7C, 0xEE, 0xC6, 0x00
	.byte 0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x18, 0x00, 0xFE, 0x0E, 0x1C, 0x38, 0x70, 0xE0, 0xFE, 0x00
_06006230:
	.byte 0x00, 0x30, 0x08, 0x38, 0x48, 0x48, 0x34, 0x00, 0x00, 0x20, 0x20, 0x38, 0x24, 0x24, 0x38, 0x00
	.byte 0x00, 0x00, 0x38, 0x40, 0x40, 0x40, 0x38, 0x00, 0x00, 0x04, 0x04, 0x1C, 0x24, 0x24, 0x1C, 0x00
	.byte 0x00, 0x00, 0x38, 0x44, 0x78, 0x40, 0x3C, 0x00, 0x00, 0x18, 0x10, 0x3C, 0x10, 0x10, 0x10, 0x00
	.byte 0x00, 0x30, 0x48, 0x3A, 0x0C, 0x14, 0x08, 0x00, 0x00, 0x20, 0x20, 0x38, 0x24, 0x24, 0x24, 0x00
	.byte 0x00, 0x00, 0x10, 0x00, 0x10, 0x10, 0x10, 0x00, 0x00, 0x08, 0x00, 0x08, 0x08, 0x48, 0x30, 0x00
	.byte 0x00, 0x40, 0x40, 0x48, 0x50, 0x70, 0x4C, 0x00, 0x00, 0x08, 0x14, 0x14, 0x14, 0x08, 0x16, 0x00
	.byte 0x00, 0x00, 0x28, 0x54, 0x54, 0x54, 0x54, 0x00, 0x00, 0x00, 0x30, 0x48, 0x48, 0x48, 0x48, 0x00
	.byte 0x00, 0x00, 0x30, 0x48, 0x48, 0x48, 0x30, 0x00, 0x00, 0x38, 0x24, 0x24, 0x38, 0x20, 0x20, 0x00
	.byte 0x00, 0x30, 0x48, 0x48, 0x38, 0x0A, 0x04, 0x00, 0x00, 0x00, 0x60, 0x1C, 0x10, 0x10, 0x10, 0x00
	.byte 0x00, 0x00, 0x38, 0x40, 0x38, 0x04, 0x78, 0x00, 0x00, 0x20, 0x70, 0x20, 0x20, 0x20, 0x18, 0x00
	.byte 0x00, 0x00, 0x48, 0x48, 0x48, 0x48, 0x34, 0x00, 0x00, 0x00, 0x44, 0x44, 0x28, 0x38, 0x10, 0x00
	.byte 0x00, 0x00, 0x44, 0x54, 0x54, 0x54, 0x28, 0x00, 0x00, 0x00, 0x44, 0x28, 0x10, 0x28, 0x44, 0x00
	.byte 0x00, 0x24, 0x24, 0x1C, 0x04, 0x24, 0x18, 0x00, 0x00, 0x00, 0x7C, 0x08, 0x10, 0x20, 0x7C, 0x00
	.byte 0x00, 0x1C, 0x22, 0x02, 0x08, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x7C, 0x00, 0x00, 0x00, 0x00
_06006310:
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xEE, 0xFF, 0xFF, 0xFF, 0xEE, 0xFE, 0xFF, 0xFF, 0xEE, 0xEE, 0xFF
	.byte 0xFF, 0xEE, 0xFE, 0xFF, 0xFF, 0xEE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
_06006350:
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x62, 0x55, 0x75, 0x03, 0x76, 0x01, 0x62, 0x04, 0x77, 0x20, 0x42, 0x04, 0x77, 0x32, 0x51, 0x05
	.byte 0x77, 0x62, 0x56, 0x05, 0x76, 0x21, 0x76, 0x01, 0x72, 0x55, 0x37, 0x07, 0x00, 0x00, 0x00, 0x00
	.byte 0x33, 0x44, 0x74, 0x03, 0x73, 0x04, 0x70, 0x07, 0x73, 0x20, 0x70, 0x07, 0x33, 0x23, 0x77, 0x03
	.byte 0x32, 0x33, 0x37, 0x02, 0x22, 0x13, 0x23, 0x02, 0x22, 0x50, 0x24, 0x02, 0x00, 0x00, 0x00, 0x00
	.byte 0x66, 0x77, 0x27, 0x00, 0x76, 0x01, 0x75, 0x02, 0x77, 0x00, 0x70, 0x07, 0x77, 0x22, 0x72, 0x05
	.byte 0x77, 0x11, 0x71, 0x07, 0x77, 0x00, 0x74, 0x03, 0x77, 0x66, 0x36, 0x01, 0x00, 0x00, 0x00, 0x00
	.byte 0x75, 0x46, 0x54, 0x01, 0x75, 0x02, 0x50, 0x05, 0x75, 0x02, 0x50, 0x05, 0x75, 0x13, 0x51, 0x05
	.byte 0x75, 0x46, 0x54, 0x01, 0x75, 0x02, 0x10, 0x01, 0x75, 0x22, 0x32, 0x03, 0x00, 0x00, 0x00, 0x00
	.byte 0x10, 0x21, 0x12, 0x01, 0x10, 0x21, 0x12, 0x01, 0x30, 0x23, 0x32, 0x03, 0x20, 0x33, 0x33, 0x02
	.byte 0x00, 0x74, 0x03, 0x00, 0x00, 0x74, 0x03, 0x00, 0x00, 0x14, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x62, 0x77, 0x77, 0x06, 0x32, 0x41, 0x14, 0x01, 0x33, 0x40, 0x04, 0x00, 0x33, 0x62, 0x26, 0x00
	.byte 0x33, 0x40, 0x04, 0x00, 0x32, 0x41, 0x14, 0x01, 0x62, 0x77, 0x77, 0x06, 0x00, 0x00, 0x00, 0x00
	.byte 0x73, 0x44, 0x74, 0x03, 0x77, 0x03, 0x71, 0x07, 0x77, 0x33, 0x71, 0x07, 0x77, 0x33, 0x73, 0x07
	.byte 0x77, 0x30, 0x72, 0x07, 0x77, 0x00, 0x72, 0x07, 0x73, 0x44, 0x74, 0x03, 0x00, 0x00, 0x00, 0x00
	.byte 0x71, 0x77, 0x57, 0x04, 0x33, 0x40, 0x36, 0x01, 0x33, 0x40, 0x14, 0x01, 0x31, 0x62, 0x37, 0x01
	.byte 0x11, 0x51, 0x25, 0x02, 0x33, 0x50, 0x35, 0x02, 0x31, 0x62, 0x37, 0x01, 0x00, 0x00, 0x00, 0x00
	.byte 0x11, 0x00, 0x10, 0x01, 0x11, 0x00, 0x10, 0x01, 0x31, 0x22, 0x32, 0x01, 0x31, 0x22, 0x32, 0x01
	.byte 0x11, 0x00, 0x10, 0x01, 0x11, 0x00, 0x10, 0x01, 0x10, 0x11, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00
_06006530:
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x1F, 0x00, 0x00, 0x00
_06006548: .4byte 0x060038CC
_0600654C:
	.byte 0x00, 0x00, 0x00, 0x00
	.byte 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00

	arm_func_start sub_06006558
sub_06006558: @ 0x06006558
	@ Copy 0x40 bytes from _06006310 to 0x06003040
	ldr r5, _0600699C @ =_06006310
	ldr r4, _060069A0 @ =0x06003040
	add r6, r5, #0x40
_06006564:
	ldm r5!, {r1, r2, r3, r11}
	stm r4!, {r1, r2, r3, r11}
	cmp r5, r6
	blo _06006564

	@ Copy 0x400 bytes from _06006350 to 0x06003080, ORing each word with 0x88888888
	add r6, r4, #0x400
	ldr r7, _060069A4 @ =0x88888888
_0600657C:
	ldm r5!, {r1, r2, r3, r11}
	orr r1, r1, r7
	orr r2, r2, r7
	orr r3, r3, r7
	orr r11, r11, r7
	stm r4!, {r1, r2, r3, r11}
	cmp r4, r6
	blo _0600657C

	mov r12, #0x4000000
	mov r11, #0x700
	strh r11, [r12, #8] @ REG_BG0CNT = 0x700 (Screen Base Block = 7)
	mov r11, #0x80000
	str r11, [r12, #0x10] @ REG_BG0HOFS = 0, REG_BG0VOFS = 8

	@ OR 0x7c0 bytes from 0x06003840 with 0x01820182
	ldr r4, _060069A8 @ =0x06003840
	add r5, r4, #0x7c0
	ldr r11, _060069AC @ =0x01820182
_060065BC:
	str r11, [r4], #4
	cmp r4, r5
	blo _060065BC

	@ Copy 24 bytes _06006530 to 0x05000008 (transfer 12 colors to first entry of PALRAM)
	mov r4, #0x5000000
	add r4, r4, #8
	sub r1, pc, #0xa8
_060065D4:
	ldr r11, [r1], #4
	str r11, [r4], #4
	tst r4, #0x1e
	bne _060065D4

	@ Write following color pattern to PALRAM, where B is black, and X is the color:
	@ BBBBBBBBBXBXBXBX
	@ BBXXBBXXBBXXBBXX
	@ BBBBXXXXBBBBXXXX
	@ This is done for white, then black (implied), then gray, then red
	mov r11, #0 @ r11 = 0 (black)
	mvn r1, #0x8000 @ r1 = 0x7FFF (white)
	mov r3, #1
_060065F0:
	add r4, r4, #0x10 @ advance r4 by 8 colors
	strh r11, [r4], #2 @ write black
	mov r5, #3
_060065FC:
	lsr r2, r4, #5
	and r2, r2, #3 @ r2 = (r4 >> 5) & 3
	tst r4, r3, lsl r2
	strhne r1, [r4], #2 @ if r4 & (1 << r2), write r1
	strheq r11, [r4], #2 @ else, write r11
	tst r4, #0x1e
	bne _060065FC @ continue until start of next palette row
	subs r5, r5, #1
	bne _060065FC
	cmp r1, #0x1f
	beq _06006648 @ exit loop when red finished
	tst r1, #1          @ if !(r1 & 1) (r1 is gray):
	moveq r1, #0x1f         @ set color to red
	addeq r4, r4, #0x20     @ advance r4 to next palette row
	beq _060065F0           @ goto _060065F0
	mov r1, #0x7300
	orr r1, r1, #0x9c @ r1 = 0x739C (gray)
	add r4, r4, #0xa0 @ advance r4 by 10 palette rows
	b _060065F0

_06006648:
	mov r6, sp @ r6 = original sp (0x03007EF8)
	mov r5, #0xb00
	orr r5, r5, #0x58
	sub sp, sp, r5
	bic sp, sp, #0x1fc @ sp = (sp - 0xB58) & ~0x1FC (0x03007200)
	mov r7, sp
	ldr r11, _060069B0 @ =sUnk_03005808
	stm r11!, {r7} @ sUnk_03005808 = 0x03007200

	@ Clear 0x03007200 to 0x03007EF8 (stack scratch)
	mov r11, #0
_0600666C:
	stm r7!, {r11}
	cmp r7, r6
	blo _0600666C

	str r6, [sp, #SP_920] @ SP_920 = r6 (original sp (0x03007EF8))
	ldr r11, _060069B4 @ =_030029AC
	str r11, [sp, #SP_8A4] @ SP_8A4 = _030029AC
	ldr r0, _060069B8 @ =_030029F0
	str r0, [sp, #SP_8A8] @ SP_8A8 = _030029F0
	ldr r0, _06006548 @ =0x060038CC
	str r0, [sp, #SP_934] @ SP_934 = 0x060038CC
	ldr r2, _06006998 @ =0x0D110001
	tst r2, #0x4000
	ldr r3, _060069BC @ =0x01010000
	eoreq r3, r3, r3, lsr #8
	str r3, [sp, #SP_8B0] @ SP_8B0 = 0x01000100
	ldr r3, _06006994 @ =0x0201C000
	str r3, [sp, #SP_854] @ SP_854 = 0x0201C000
	mov r4, #0x6000000
	ldr r6, _060069C0 @ =0x0203C000
	str r6, [sp, #SP_84C] @ SP_84C = 0x0203C000
	sub r6, r6, #0x6000
	orr r6, r4, r6, lsr #8
	str r6, [sp, #SP_858] @ SP_858 = 0x06020360
	str r6, [r11, #0xc] @ _030029B8 = 0x06020360
	sub r5, r3, #0x8000
	orr r5, r4, r5, lsr #8
	str r5, [sp, #SP_85C] @ SP_85C = 0x06020140
	str r5, [r11, #0x10] @ _030029BC = 0x06020140
	str r5, [r11, #0x14] @ _030029C0 = 0x06020140
	sub r5, r5, #0x20
	str r5, [sp, #SP_860] @ SP_860 = 0x06020120
	orr r6, r4, sp, lsr #8
	str r6, [r11] @ _030029AC = 0x06030072
	sub r5, r5, #0x20
	str r5, [sp, #SP_864] @ SP_864 = 0x06020100
	sub r6, r6, #0x100
	str r6, [r11, #0x20] @ _030029CC = 0x0602FF72
	sub r5, r5, #0x20
	str r5, [sp, #SP_868] @ SP_868 = 0x060200E0
	lsr r0, r2, #0x10
	mov r1, #1
	lsl r4, r1, r0
	str r4, [sp, #SP_870] @ SP_870 = 0x00020000
	sub r5, r5, #0x20
	add r5, r5, r4, lsr #8
	str r5, [r11, #0x18] @ _030029C4 = 0x060202C0
	str r5, [r11, #0x1c] @ _030029C8 = 0x060202C0
	lsl r6, r5, #8
	add r6, r6, #0xc000
	str r6, [sp, #SP_91C] @ SP_91C = 0x02038000
	cmp r4, #0x80000
	addne r5, r5, #0x400
	str r5, [sp, #SP_86C] @ SP_86C = 0x060206C0
	sub r5, r4, #1
	lsr r5, r5, #8
	str r5, [sp, #SP_874] @ SP_874 = 0x000001FF
	lsr r0, r0, #8
	lsl r5, r1, r0
	str r5, [sp, #SP_878] @ SP_878 = 0x00002000
	sub r6, r5, #1
	lsr r6, r6, #0xa
	str r6, [sp, #SP_87C] @ SP_87C = 0x00000007
	ldr r6, _06006990 @ =0x0203C000
	str r6, [sp, #SP_844] @ SP_844 = 0x0203C000
	add r6, r6, r5
	str r6, [sp, #SP_848] @ SP_848 = 0x0203E000
	tst r2, #0x8000 @ this tst is always false here
	ldr r7, _060069C4 @ =_03002E68
	ldr r8, _0600698C @ =0xE1A0F00E
	strne r8, [r7]
	and r0, r2, #0xf00
	lsr r0, r0, #8
	strb r0, [sp, #SP_A38] @ SP_A38 = 0x00
	ldr r3, _060069C8 @ =0x44444444
	str r3, [sp, #SP_8FC] @ SP_8FC = 0x44444444
	ldr r3, _060069CC @ =sub_06006000
	str r3, [sp, #SP_900] @ SP_900 = sub_06006000
	strb r2, [sp, #SP_A4E] @ SP_A4E = 0x01
	mov r0, #0x6800
	orr r0, r0, #0x6000000
	str r0, [sp, #SP_8EC] @ SP_8EC = 0x06006800
	add r1, r0, #0xe00
	add r1, r1, r0
	str r1, [sp, #SP_8F8] @ SP_8F8 = 0x0C00DE00
	mvn r0, #0xf3
	eor r0, r0, #0x360
	add r1, sp, #0x200
	stmdb r1!, {r0} @ SP_1FC (0x030073FC) = 0xFFFFFC6C (-0x394)
	str r1, [sp, #SP_88C] @ SP_88C = SP_1FC (0x030073FC)
	add r0, r1, #3
	str r0, [sp, #SP_890] @ SP_890 = 0x030073FF
	mov r0, #0x820000
	str r0, [sp, #SP_8AC] @ SP_8AC = 0x00820000
	ldr r0, _060069D0 @ =EmulatorAudio_WriteToApu
	str r0, [sp, #SP_9B4] @ SP_9B4 = EmulatorAudio_WriteToApu
	mov r0, #0xef
	str r0, [sp, #SP_8B4] @ SP_8B4 = 0x000000EF
	ldr r0, _060069D4 @ =0x03020100
	ldr r1, _060069D8 @ =0x07060504
	str r0, [sp, #SP_8E4] @ SP_8E4 = 0x03020100
	str r1, [sp, #SP_8E8] @ SP_8E8 = 0x07060504
	str r0, [sp, #SP_95C] @ SP_95C = 0x03020100
	str r1, [sp, #SP_960] @ SP_960 = 0x07060504
	eor r2, r0, r1 @ r2 = 0x04040404
	eor r0, r0, r2, lsl #1
	eor r1, r1, r2, lsl #1
	str r0, [sp, #SP_964] @ SP_964 = 0x0B0A0908
	str r1, [sp, #SP_968] @ SP_968 = 0x0F0E0D0C

	@ Set SP_8C4 to SP_8E3 to 0x1E
	add r1, sp, #0x20
	mov r0, #0x1e
_06006824:
	sub r1, r1, #1
	strb r0, [r1, #SP_8C4]
	cmp r1, sp
	bne _06006824

	@ Set SP_94C to SP_96B to 0xFF
	mvn r0, #0
	add r1, sp, #0xf
_0600683C:
	strb r0, [r1, #SP_94C]
	strb r0, [r1, #SP_95C]
	sub r1, r1, #1
	cmp r1, sp
	bhs _0600683C

	mov r0, #0x40
	strb r0, [sp, #SP_A3C] @ SP_A3C = 0x40
	lsl r0, r0, #8
	str r0, [sp, #SP_9D0] @ SP_9D0 = 0x00004000
	str r0, [sp, #SP_9D4] @ SP_9D4 = 0x00004000
	ldr r0, _060069DC @ =sub_03005568
	str r0, [sp, #SP_9BC] @ SP_9BC = sub_03005568
	sub lr, lr, #4
	str lr, [sp, #SP_924] @ SP_924 = 0x087D8000
	ldr r1, _060069E0 @ =sub_030057A8
	ldr r2, _060069E4 @ =0x03007000
	str r1, [r2, #0xffc] @ 0x03007FFC = sub_030057A8 (Interrupt handler)

_06006880:
	bl sub_0600E754
_06006884:
	mov r0, #0
	strb r0, [sp, #SP_A2F] @ SP_A2F = r0 = 0x00
	bl sub_0600E4D0
	strb r0, [sp, #SP_A2E] @ SP_A2E = r0 = sub_0600E4D0(0)
	cmp r0, #2    @ if r0 >= 2: 
	ldr r0, _060069E8 @ =0x0E007FD8
	bhs _060068D0     @ goto _060068D0
	bl sub_0600E4D0
	strb r0, [sp, #SP_A2F] @ SP_A2F = r0 = sub_0600E4D0(0x0E007FD8)
	cmp r0, #2    @ if r0 >= 2:
	bhs _060068BC     @ goto _060068BC
	ldrb r1, [sp, #SP_A2E]
	eors r0, r0, r1 @ if r0 ^ SP_A2E != 0:
	bne _0600694C       @ goto _0600694C
_060068BC:
	mov r0, #0
	bl sub_0600E4D0 @ r0 = sub_0600E4D0(0)
	cmp r0, #2    @ if r0 < 2:
	blo _0600694C     @ goto _0600694C
	b _06006884   @ else goto _06006884

_060068D0:
	bl sub_0600E4D0
	strb r0, [sp, #SP_A2F] @ SP_A2F = r0 = sub_0600E4D0(0x0E007FD8)
	cmp r0, #2    @ if r0 < 2:
	blo _0600694C     @ goto _0600694C
	cmp r0, #3               @ if r0 < 3:
	ldrblo r0, [sp, #SP_A2E]     @ r0 = SP_A2E
	cmplo r0, #3
	mov r0, #0 @ r0 = 0
	strb r0, [sp, #SP_A2E] @ SP_A2E = 0
	strb r0, [sp, #SP_A2F] @ SP_A2F = 0
	ldr r1, _06006948 @ =0x0600A000
	str r0, [r1] @ 0x0600A000 = 0

	@ Clear 0x2000 bytes of SP_84C (NES WRAM area)
	ldr r1, [sp, #SP_84C]
	add r2, r1, #0x2000
_06006908:
	str r0, [r1], #4
	teq r1, r2
	bne _06006908

	blo _06006964 @ if r1 < r2 goto _06006964 (always true?)
	mov r0, #0x16
	bl sub_06006E08
_06006920:
	bl sub_06007210
	bne _06006920
_06006928:
	bl sub_06007210
	tst r0, #0xf
	beq _06006928
_06006934:
	bl sub_06007210
	bne _06006934
	mov r0, #0x80
	bl sub_06006E08
	b _06006964
	.align 2, 0
_06006948: .4byte 0x0600A000

_0600694C:
	ldr r0, [sp, #SP_854]
	ldr r3, _06006948 @ =0x0600A000
	stm r4, {r0, r3, r5} @ DMA(src=SP_854, dst=0x0600A000, cnt=r5(always Enable, 32bit, 0x800 bytes?))
	@ r0 = EmulatorSaveToPasswordBytes(SP_854) (returns input as output)
	add lr, pc, #0x4 @ =_06006964
	ldr r12, _060069EC @ =EmulatorSaveToPasswordBytes
	bx r12

_06006964:
	bl sub_0600E794 @ sub_0600E794()
_06006968:
	mov r12, #0x4000000
	mov r0, #0
	strh r0, [r12] @ REG_DISPCNT = 0 (Display off)
	@ sub_0600E048(0x920, SP)
	@ goto sub_0600C00C() (does not return)
	mov r0, #0x900
	orr r0, r0, #0x20
	mov r1, sp
	ldr r2, _060069F0 @ =sub_0600E048
	ldr lr, _060069F4 @ =sub_0600C00C
	bx r2
	.align 2, 0
_0600698C: .4byte 0xE1A0F00E
_06006990: .4byte 0x0203C000
_06006994: .4byte 0x0201C000
_06006998: .4byte 0x0D110001
_0600699C: .4byte _06006310
_060069A0: .4byte 0x06003040
_060069A4: .4byte 0x88888888
_060069A8: .4byte 0x06003840
_060069AC: .4byte 0x01820182
_060069B0: .4byte sUnk_03005808
_060069B4: .4byte _030029AC
_060069B8: .4byte _030029F0
_060069BC: .4byte 0x01010000
_060069C0: .4byte 0x0203C000
_060069C4: .4byte _03002E68
_060069C8: .4byte 0x44444444
_060069CC: .4byte sub_06006000
_060069D0: .4byte EmulatorAudio_WriteToApu
_060069D4: .4byte 0x03020100
_060069D8: .4byte 0x07060504
_060069DC: .4byte sub_03005568
_060069E0: .4byte sub_030057A8
_060069E4: .4byte 0x03007000
_060069E8: .4byte 0x0E007FD8
_060069EC: .4byte EmulatorSaveToPasswordBytes
_060069F0: .4byte sub_0600E048
_060069F4: .4byte sub_0600C00C

@ TODO: file split?
_060069F8:
	.zero 0x408

_06006E00: .4byte 0x06003840
_06006E04: .4byte 0x01820182

	@ Draws text to the screen
	@ r0 = Text ID
	arm_func_start sub_06006E08
sub_06006E08: @ 0x06006E08
	tst r0, #0x80 @ if (r0 & 0x80) == 0:
	beq _06006E24     @ goto _06006E24

	@ Fill 0x06003840 to 0x06003FFF with 0x01820182
	@ This clears the text
	ldr r1, _06006E00 @ =0x06003840
	ldr r2, _06006E04 @ =0x01820182
_06006E18:
	str r2, [r1], #4
	tst r1, #0x800
	bne _06006E18

	@ Skip _06006ED0 if r0 is:
	@ 1, 2, 0x11-0x16, 0x18, 0x1a-0x1e
	@ Dont skip _06006ED0 if r0 is:
	@ 3-0x10, 0x17, 0x19, 0x20
	@ or has 0x40 flag
_06006E24:
	tst r0, #0x40
	bic r0, r0, #0xc0
	add r5, pc, #0x11C @ =_06006F50
	bne _06006ED0
	cmp r0, #3
	cmpne r0, #4
	add r5, pc, #0x114 @ =_06006F58
	beq _06006ED0
	cmp r0, #6
	add r5, pc, #0x110 @ =_06006F60
	beq _06006ED0
	cmp r0, #7
	add r5, pc, #0x10C @ =_06006F68
	beq _06006ED0
	cmp r0, #8
	add r5, pc, #0x108 @ =_06006F70
	beq _06006ED0
	cmp r0, #0xa
	add r5, pc, #0x10C @ =_06006F80
	beq _06006ED0
	cmp r0, #0x17
	add r5, pc, #0x108 @ =_06006F88
	cmpne r0, #5
	beq _06006ED0
	cmp r0, #0xe
	cmpne r0, #0xf
	add r5, pc, #0x104 @ =_06006F98
	beq _06006ED0
	cmp r0, #0x10
	add r5, pc, #0x100 @ =_06006FA0
	beq _06006ED0
	cmp r0, #0x19
	cmpne r0, #0x20
	add r5, pc, #0xF8 @ =_06006FA8
	beq _06006ED0
	cmp r0, #0xb
	add r5, pc, #0xD4 @ =_06006F90
	cmpne r0, #0xc
	cmpne r0, #0xd
	beq _06006ED0
	cmp r0, #9
	add r5, pc, #0xA8 @ =_06006F78
	bne _06006EF8

	@ r5 is pointer to data
	@ data is structured as:
	@     .4byte dst_addr
	@     .2byte num_col
	@     .2byte num_row
	@ Copy 0x1184 into num_col x num_row at dst_addr
	@ TODO: what does this accomplish?
_06006ED0:
	ldr r1, _06006FFC @ =0x00001184
	ldr r2, [r5]
	ldrh r4, [r5, #6]
_06006EDC:
	ldrh r3, [r5, #4]
_06006EE0:
	strh r1, [r2, r3]
	subs r3, r3, #2
	bhs _06006EE0
	add r2, r2, #0x40
	subs r4, r4, #1
	bhs _06006EDC

_06006EF8:
	mov r12, lr
	mov r4, #0x1000 @ r4 = 0x1000 (white)
	mov r5, #0x100
	orr r5, r5, #0x84 @ r5 = 0x184 (tile index base)
	ands r0, r0, #0x3f @ r0 &= ~0xC0
	bxeq r12 @ if r0 == 0: return
	add r3, pc, r0, lsl #2
	ldr r3, [r3, #0xe8] @ r3 = _06007004[r0 - 1]

	@ Copy text onto screen until 0xFFFF
_06006F18:
	ldr r2, [r3], #4 @ r2 = *r3++
	cmp r2, #0       @ if r2 < 0:
	bxmi r12             @ return

	@ Call sub_06006FB0() on each byte
	@ Exit loop if byte is 0
_06006F24:
	ldrh r1, [r3], #2
	ands r0, r1, #0xff
	beq _06006F44
	bl sub_06006FB0
	lsrs r0, r1, #8
	beq _06006F44
	bl sub_06006FB0
	b _06006F24

	@ Inc to next entry
_06006F44:
	add r3, r3, #3
	bic r3, r3, #3
	b _06006F18

	@ Used by 0x40 flag
_06006F50:
	.4byte 0x060038CC
	.2byte 0x22
	.2byte 0xF

	@ Used by 3 and 4
_06006F58:
	.4byte 0x06003A12
	.2byte 0x16
	.2byte 0x2

	@ Used by 6
_06006F60:
	.4byte 0x060038CA
	.2byte 0x26
	.2byte 0xF

	@ Used by 7
_06006F68:
	.4byte 0x060038CA
	.2byte 0x26
	.2byte 0xF

	@ Used by 8
_06006F70:
	.4byte 0x060038CA
	.2byte 0x26
	.2byte 0xE

	@ Used by 9
_06006F78:
	.4byte 0x060039D0
	.2byte 0x1A
	.2byte 0x4

	@ Used by 0xA
_06006F80:
	.4byte 0x060038CA
	.2byte 0x26
	.2byte 0xE

	@ Used by 5 and 0x17
_06006F88:
	.4byte 0x0600398A
	.2byte 0x26
	.2byte 0xB

	@ Used by 0xB, 0xC, and 0xD
_06006F90:
	.4byte 0x06003B8C
	.2byte 0x22
	.2byte 0x5

	@ Used by 0xE and 0xF
_06006F98:
	.4byte 0x06003ACC
	.2byte 0x22
	.2byte 0x6

	@ Used by 0x10
_06006FA0:
	.4byte 0x06003840
	.2byte 0x3A
	.2byte 0x13

	@ Used by 0x19 and 0x20
_06006FA8:
	.4byte 0x06003A00
	.2byte 0x3A
	.2byte 0x8

	@ Draw text character
	@ r0 = Tile text
	@ r2 = Dst address
	@ r4 = Color
	@ r5 = Tile index base
	arm_func_start sub_06006FB0
sub_06006FB0: @ 0x06006FB0
	subs r0, r0, #0x20 @ r0 -= 0x20
	blo _06006FD4 @ if r0 < 0: goto _06006FD4
	orr r0, r0, r0, lsl #7 @ r0 = r0 | (r0 << 7)
	bic r0, r0, #0xf80 @ r0 &= ~0xF80
	bic r0, r0, #0x60 @ r0 &= ~0x60
	add r0, r0, r4 @ r0 += r4 (color)
	add r0, r0, r5 @ r0 += r5 (tile index base)
	strh r0, [r2], #2 @ *r2++ = r0
	bx lr

	@ Handles text commands
_06006FD4:
	cmn r0, #0x1f     @ if r0 + 0x1F == 0 (r0 was 1):
	moveq r4, #0xd000     @ r4 = 0xD000 (red color)
	cmn r0, #0x1e     @ if r0 + 0x1E == 0 (r0 was 2):
	moveq r4, #0x1000     @ r4 = 0x1000 (white color)
	cmn r0, #0x1d     @ if r0 + 0x1D == 0 (r0 was 3):
	moveq r4, #0x9000     @ r4 = 0x9000 (gray color)
	cmn r0, #0x1b     @ if r0 + 0x1B == 0 (r0 was 5):
	ldreq r0, _06007000 @ =0x00000183
	strheq r0, [r2], #2   @ *r2++ = 0x183 (red selection arrow)
	bx lr
	.align 2, 0
_06006FFC: .4byte 0x00001184
_06007000: .4byte 0x00000183
_06007004:
	.4byte sEmulatorMenuText                @ 0x01
	.4byte sEmulatorErrorText               @ 0x02
	.4byte sEmulatorSleepModeText           @ 0x03
	.4byte 0x00000000                       @ 0x04
	.4byte sEmulatorSleepModeText           @ 0x05
	.4byte sEmulatorSleepModeText           @ 0x06
	.4byte sEmulatorSleepModeText           @ 0x07
	.4byte sEmulatorSleepModeText           @ 0x08
	.4byte sEmulatorSleepModeText           @ 0x09
	.4byte sEmulatorSleepModeText           @ 0x0A
	.4byte sEmulatorSleepModeText           @ 0x0B
	.4byte sEmulatorSleepModeText           @ 0x0C
	.4byte sEmulatorSleepModeText           @ 0x0D
	.4byte sEmulatorSleepModeText           @ 0x0E
	.4byte sEmulatorSleepModeText           @ 0x0F
	.4byte sEmulatorEmptyText               @ 0x10
	.4byte sEmulatorSleepModeText           @ 0x11
	.4byte sEmulatorSleepModeText           @ 0x12
	.4byte sEmulatorSleepModeText           @ 0x13
	.4byte sEmulatorSleepModeText           @ 0x14
	.4byte sEmulatorSleepModeText           @ 0x15
	.4byte sEmulatorSleepModeText           @ 0x16
	.4byte sEmulatorSleepModeText           @ 0x17
	.4byte sEmulatorSleepModeText           @ 0x18
	.4byte sEmulatorQuitMenuText            @ 0x19
	.4byte sEmulatorMenuSelectYesText       @ 0x1A
	.4byte sEmulatorMenuSelectNoText        @ 0x1B
	.4byte sEmulatorGameOverSaveRequestText @ 0x1C
	.4byte sEmulatorGameOverSelectYesText   @ 0x1D
	.4byte sEmulatorGameOverSelectNoText    @ 0x1E
	.4byte sEmulatorGameOverContinueText    @ 0x1F
	.4byte sEmulatorResetMenuText           @ 0x20

sEmulatorMenuText: @ 0x06007084
	@ - MENU -
	.4byte 0x06003916
	.byte 0x4E, 0x20, 0x2C, 0x4B, 0x4C, 0x2E, 0x20, 0x4E
	.byte 0x00, 0x00, 0x00, 0x00

	@ CONTINUE
	.4byte 0x06003996
	.byte 0x03, 0x2B, 0x6C, 0x4C, 0x6D, 0x6B, 0x4C, 0x2E, 0x4B
	.byte 0x00, 0x00, 0x00

	@ QUIT
	.4byte 0x06003A16
	.byte 0x65, 0x2E, 0x6B, 0x6D
	.byte 0x00, 0x00, 0x00, 0x00

	@ RESET
	.4byte 0x06003A96
	.byte 0x2D, 0x4B, 0x4D, 0x4B, 0x6D
	.byte 0x00, 0x00, 0x00

	@ SLEEP
	.4byte 0x06003B16
	.byte 0x4D, 0x48, 0x4B, 0x4B, 0x68
	.byte 0x00, 0x00, 0x00

	@ TO DISABLE SLEEP
	.4byte 0x06003B8E
	.byte 0x02, 0x6D, 0x6C, 0x20, 0x67, 0x6B, 0x4D, 0x27, 0x47, 0x48, 0x4B, 0x20, 0x4D, 0x48, 0x4B, 0x4B
	.byte 0x68
	.byte 0x00, 0x00, 0x00

	@ MODE, PRESS
	.4byte 0x06003BCE
	.byte 0x2C, 0x6C, 0x67, 0x4B, 0x69, 0x20, 0x68, 0x2D, 0x4B, 0x4D, 0x4D
	.byte 0x00

	@ L+R+SELECT
	.4byte 0x06003C14
	.byte 0x01, 0x48, 0x49, 0x2D, 0x49, 0x4D, 0x4B, 0x48, 0x4B, 0x2B, 0x6D
	.byte 0x00

	@ SIMULTANEOUSLY
	.4byte 0x06003C4E
	.byte 0x02, 0x4D, 0x6B, 0x2C, 0x2E, 0x48, 0x6D, 0x27, 0x4C, 0x4B, 0x6C, 0x2E, 0x4D, 0x48, 0x29, 0x2A
	.byte 0x00, 0x00, 0x00, 0x00

	.4byte 0xFFFFFFFF

sEmulatorErrorText: @ 0x0600711C
	@ CARTRIDGE
	@   ERROR
	.4byte 0x06003A16
	.byte 0x2B, 0x27, 0x2D, 0x6D, 0x2D, 0x6B, 0x67, 0x25, 0x4B, 0x00, 0x00, 0x00, 0x9A, 0x3A, 0x00, 0x06
	.byte 0x01, 0x4B, 0x2D, 0x2D, 0x6C, 0x2D
	.byte 0x00, 0x00
	.4byte 0xFFFFFFFF

	arm_func_start sub_0600713C
sub_0600713C: @ 0x0600713C
	mov r0, #0x51
	bl sub_06006E08
	mov r0, #0x12
	bl sub_06006E08
	mov r0, #0
	strb r0, [sp, #SP_A43]
_06007154:
	bl sub_06007210
	beq _06007154
	cmp r0, #2
	moveq r0, #0
	strbeq r0, [sp, #SP_A43]
	beq _060071F0
	cmp r0, #0x10
	bne _06007188
	mov r0, #0
	strb r0, [sp, #SP_A43]
	mov r0, #0x12
	bl sub_06006E08
	b _06007154
_06007188:
	cmp r0, #0x20
	bne _060071A0
	strb r0, [sp, #SP_A43]
	mov r0, #0x13
	bl sub_06006E08
	b _06007154
_060071A0:
	cmp r0, #1
	bne _06007154
	ldrb r0, [sp, #SP_A43]
	cmp r0, #0
	beq _060071F0
	add lr, pc, #0x8 @ =_060071C4
	ldr r12, _06007234 @ =sub_0203E3AC
	mov r0, sp
	bx r12
_060071C4:
	cmp r0, #0
	moveq r0, #0x54
	movne r0, #0
	strbne r0, [sp, #SP_A43]
	movne r0, #0x55
	bl sub_06006E08
_060071DC:
	bl sub_06007210
	bne _060071DC
_060071E4:
	bl sub_06007210
	tst r0, #0xf
	beq _060071E4
_060071F0:
	bl sub_06007210
	bne _060071F0
	mov r0, #0x80
	bl sub_06006E08
	ldrb r0, [sp, #SP_A43]
	cmp r0, #0
	beq _06006880
	bne _06006968

	arm_func_start sub_06007210
sub_06007210: @ 0x06007210
	mov r0, #0x44000
_06007214:
	subs r0, r0, #9
	bpl _06007214
	ldr r12, _06007238 @ =0x04000130
	ldr r0, [r12]
	ldr r1, _0600723C @ =0x000003FF
	and r0, r0, r1
	eors r0, r0, r1
	bx lr
	.align 2, 0
_06007234: .4byte sub_0203E3AC
_06007238: .4byte 0x04000130
_0600723C: .4byte 0x000003FF
