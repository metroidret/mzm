#include "data/animated_graphics_data.h"
#include "macros.h"

#include "data/animated_tiles_data.h"
#include "data/tilesets/tilesets_set0.h"
#include "data/tilesets/tilesets_set1.h"
#include "data/tilesets/tilesets_set2.h"
#include "data/tilesets/tilesets_set3.h"
#include "data/tilesets/tilesets_set4.h"
#include "data/tilesets/tilesets_set5.h"
#include "data/tilesets/tilesets_set6.h"
#include "data/tilesets/tilesets_set7.h"

#include "constants/audio.h"


const struct AnimatedGraphicsData sAnimatedGraphicsEntries[ANIMATED_GFX_ID_COUNT] = {
    [ANIMATED_GFX_ID_CRYSTAL] = {
        .type = ANIMATED_GFX_TYPE_NONE,
        .framesPerState = 0,
        .numberOfStates = 0,
        .pGraphics = &sAnimatedTilesGfx[0 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WATER_SURFACE] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.2f),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[36 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WATER_DEPTH_1] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.2f),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[40 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WATER_DEPTH_2] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.2f),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[44 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WATER_DEPTH_3] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.2f),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[48 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_LAVA_BUBBLES_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[0 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_LAVA_SURFACE_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[1 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_LAVA_SHALLOW_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[2 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_LAVA_DEPTH_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[3 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_WEAK_ACID_SURFACE_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[6 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_WEAK_ACID_DEPTH_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[7 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_LAVA_DEPTH_BUBBLES] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[4 * ANIMATED_GFX_TANK_SIZE],
    },
    [12] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[5 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_RAIN] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 30),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[4 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_RAIN_SPLASH] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 30),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[12 * ANIMATED_GFX_SIZE],
    },
    [15] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[6 * ANIMATED_GFX_TANK_SIZE],
    },
    [16] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[7 * ANIMATED_GFX_TANK_SIZE],
    },
    [17] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[8 * ANIMATED_GFX_TANK_SIZE],
    },
    [18] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[60 * ANIMATED_GFX_SIZE],
    },
    [19] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[64 * ANIMATED_GFX_SIZE],
    },
    [20] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[68 * ANIMATED_GFX_SIZE],
    },
    [21] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[72 * ANIMATED_GFX_SIZE],
    },
    [22] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[68 * ANIMATED_GFX_SIZE],
    },
    [23] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[0 * ANIMATED_GFX_TANK_SIZE],
    },
    [24] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[1 * ANIMATED_GFX_TANK_SIZE],
    },
    [25] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[2 * ANIMATED_GFX_TANK_SIZE],
    },
    [26] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[3 * ANIMATED_GFX_TANK_SIZE],
    },
    [27] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[52 * ANIMATED_GFX_SIZE],
    },
    [28] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[56 * ANIMATED_GFX_SIZE],
    },
    [29] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.2f) - DELTA_TIME,
        .numberOfStates = 4,
        .pGraphics = &sAnimatedTilesGfx[52 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WEAK_ACID_FALL_LEFT] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(2.f / 15),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[9 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_WEAK_ACID_FALL_MIDDLE] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(2.f / 15),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[10 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_WEAK_ACID_FALL_RIGHT] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(2.f / 15),
        .numberOfStates = 4,
        .pGraphics = &sAnimatedLiquidsGfx[11 * ANIMATED_GFX_TANK_SIZE],
    },
    [ANIMATED_GFX_ID_ALARM_SLOW] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 60),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[76 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_ALARM_FAST] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 60),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[84 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_ALARM_HEAVY] = {
        .type = ANIMATED_GFX_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 60),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[92 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WIND] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 60),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[20 * ANIMATED_GFX_SIZE],
    },
    [ANIMATED_GFX_ID_WIND_FAST] = {
        .type = ANIMATED_GFX_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(1.f / 30),
        .numberOfStates = 8,
        .pGraphics = &sAnimatedTilesGfx[28 * ANIMATED_GFX_SIZE],
    }
};

#define GFX_ANIM_TO_3BYTES(n) n, 0, 0
const u8 sAnimatedTilesetEntries[8][3*2*8] = {
    [0] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    },
    [1] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WATER_SURFACE), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WATER_DEPTH_1),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WATER_DEPTH_2), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WATER_DEPTH_3)
    },
    [2] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_BUBBLES_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_SURFACE_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_SHALLOW_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_DEPTH_FAST),
        GFX_ANIM_TO_3BYTES(23), GFX_ANIM_TO_3BYTES(24),
        GFX_ANIM_TO_3BYTES(25), GFX_ANIM_TO_3BYTES(26),
        GFX_ANIM_TO_3BYTES(11), GFX_ANIM_TO_3BYTES(27),
        GFX_ANIM_TO_3BYTES(29), GFX_ANIM_TO_3BYTES(28),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    },
    [3] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_SURFACE_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_DEPTH_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_SURFACE_SLOW), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_DEPTH_SLOW),
        GFX_ANIM_TO_3BYTES(17), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_FALL_LEFT),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_FALL_MIDDLE), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_FALL_RIGHT),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_SURFACE_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_DEPTH_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_SURFACE_SLOW), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WEAK_ACID_DEPTH_SLOW)
    },
    [4] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_RAIN), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_RAIN_SPLASH),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    },
    [5] = {
        GFX_ANIM_TO_3BYTES(18), GFX_ANIM_TO_3BYTES(19),
        GFX_ANIM_TO_3BYTES(20), GFX_ANIM_TO_3BYTES(21),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    },
    [6] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_ALARM_SLOW), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_ALARM_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_ALARM_HEAVY), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    },
    [7] = {
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_BUBBLES_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_SURFACE_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_SHALLOW_FAST), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_LAVA_DEPTH_FAST),
        GFX_ANIM_TO_3BYTES(23), GFX_ANIM_TO_3BYTES(24),
        GFX_ANIM_TO_3BYTES(25), GFX_ANIM_TO_3BYTES(26),
        GFX_ANIM_TO_3BYTES(11), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WIND), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_WIND_FAST),
        GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL), GFX_ANIM_TO_3BYTES(ANIMATED_GFX_ID_CRYSTAL)
    }
};

const struct AnimatedPaletteTiming sAnimatedPaletteTiming_Empty = {
    .timer1 = 0,
    .row1 = 0,

    .timer2 = 0,
    .row2 = 0
};

const struct AnimatedPaletteData sAnimatedPaletteEntries[ANIMATED_PALETTE_ID_COUNT] = {
    [0] = {
        .type = ANIMATED_PALETTE_TYPE_NONE,
        .framesPerState = 0,
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_0) / 16,
        .pPalette = sAnimatedPal_0
    },
    [1] = {
        .type = ANIMATED_PALETTE_TYPE_NORMAL,
        .framesPerState = CONVERT_SECONDS(.1f + 1.f / 60),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_0) / 16,
        .pPalette = sAnimatedPal_0
    },
    [2] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.1f - 1.f / 60),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_2) / 16,
        .pPalette = sAnimatedPal_2
    },
    [ANIMATED_PALETTE_ID_ZIPLINE] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_3) / 16,
        .pPalette = sAnimatedPal_3
    },
    [4] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_4) / 16,
        .pPalette = sAnimatedPal_4
    },
    [5] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_5) / 16,
        .pPalette = sAnimatedPal_5
    },
    [6] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_6) / 16,
        .pPalette = sAnimatedPal_6
    },
    [7] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_7) / 16,
        .pPalette = sAnimatedPal_7
    },
    [8] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_8) / 16,
        .pPalette = sAnimatedPal_8
    },
    [9] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_9) / 16,
        .pPalette = sAnimatedPal_9
    },
    [10] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_10) / 16,
        .pPalette = sAnimatedPal_10
    },
    [11] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_11) / 16,
        .pPalette = sAnimatedPal_11
    },
    [12] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_12) / 16,
        .pPalette = sAnimatedPal_12
    },
    [13] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 15),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_13) / 16,
        .pPalette = sAnimatedPal_13
    },
    [14] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_14) / 16,
        .pPalette = sAnimatedPal_14
    },
    [15] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(1.f / 6),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_15) / 16,
        .pPalette = sAnimatedPal_15
    },
    [16] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_16) / 16,
        .pPalette = sAnimatedPal_16
    },
    [17] = {
        .type = ANIMATED_PALETTE_TYPE_ALTERNATE,
        .framesPerState = CONVERT_SECONDS(.05f),
        .numbersOfStates = ARRAY_SIZE(sAnimatedPal_17) / 16,
        .pPalette = sAnimatedPal_17
    }
};


const struct BackgroundEffectColorData sBackgroundEffectColorData[BACKGROUND_EFFECT_COUNT] = {
    [BACKGROUND_EFFECT_NONE] = {
        .colorMask = COLOR_WHITE,
        .color_1 = BACKGROUND_EFFECT_NO_COLOR,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = FALSE
    },
    [BACKGROUND_EFFECT_LIGHTNING] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_WHITE,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = FALSE
    },
    [BACKGROUND_EFFECT_SLIGHT_YELLOW] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR(22, 22, 0),
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = FALSE
    },
    [BACKGROUND_EFFECT_HEAVY_YELLOW] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR(22, 22, 0),
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = FALSE
    },
    [BACKGROUND_EFFECT_EXIT_ZEBES_FADE] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_WHITE,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = TRUE
    },
    [BACKGROUND_EFFECT_INTRO_TEXT_FADE] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_BLACK,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = TRUE
    },
    [BACKGROUND_EFFECT_QUICK_FLASH] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_WHITE,
        .color_2 = COLOR_BLUE,
        .applyToObj = FALSE
    },
    [BACKGROUND_EFFECT_ALL_BLACK] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_BLACK,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = TRUE
    },
    [BACKGROUND_EFFECT_ALL_WHITE] = {
        .colorMask = COLOR_WHITE,
        .color_1 = COLOR_WHITE,
        .color_2 = BACKGROUND_EFFECT_NO_COLOR,
        .applyToObj = TRUE
    }
};


const u16 sBackgroundEffectBehavior_Lightning[34][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_RANDOM,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(5.f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_PLAY_SOUND,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = SOUND_THUNDER
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 11
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 10
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.05f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 9
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 7
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 5
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 1
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 12),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_QuickFlash[12][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 16
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_SECOND_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 16
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 14
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_AND_KILL,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_SlightYellow[21][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 13,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 5
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 5
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 10,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 10,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_AND_KILL,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_HeavyYellow[20][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 10
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(.2f),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_AND_KILL,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_ExitZebes[30][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 1
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 7
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 9
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 10
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 11
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 13
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 14
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 15
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 30),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 16
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_EXIT_ZEBES,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_IntroText[30][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 1
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 7
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 9
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 10
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 11
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 13
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 14
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 15
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 16
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_BEFORE_INTRO_TEXT,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};

const u16 sBackgroundEffectBehavior_AllBlackWhite[39][BACKGROUND_EFFECT_BEHAVIOR_FIELD_COUNT] = {
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 1
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 2
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 3
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 6
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 7
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 9
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 10
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 11
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 13
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 14
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 15
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 16
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 60,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 12
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 8
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 4
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_WAIT_FOR_TIMER_BEFORE,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = CONVERT_SECONDS(1.f / 15),
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_CHECK_APPLY_FIRST_COLOR,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    },
    {
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TYPE] = BACKGROUND_EFFECT_CMD_FINISH_AND_KILL,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_TIMER] = 0,
        [BACKGROUND_EFFECT_BEHAVIOR_FIELD_CMD_ARG] = 0
    }
};
