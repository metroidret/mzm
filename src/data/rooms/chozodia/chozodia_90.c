#include "data/rooms/chozodia_rooms_data.h"
#include "macros.h"

const u8 sChozodia_54_Scrolls[SCROLL_DATA_SIZE(4)] = {
	90, // Room
	4, // Number of scrolls
	
	// Scroll 0
	2, 61, // X bounds
	2, 11, // Y bounds
	UCHAR_MAX, UCHAR_MAX, // Breakable block position
	UCHAR_MAX, // Breakable block direction
	UCHAR_MAX, // Breakable block Y bound extension

	// Scroll 1
	17, 41, // X bounds
	2, 31, // Y bounds
	UCHAR_MAX, UCHAR_MAX, // Breakable block position
	UCHAR_MAX, // Breakable block direction
	UCHAR_MAX, // Breakable block Y bound extension

	// Scroll 2
	17, 58, // X bounds
	29, 41, // Y bounds
	UCHAR_MAX, UCHAR_MAX, // Breakable block position
	UCHAR_MAX, // Breakable block direction
	UCHAR_MAX, // Breakable block Y bound extension

	// Scroll 3
	47, 61, // X bounds
	22, 31, // Y bounds
	UCHAR_MAX, UCHAR_MAX, // Breakable block position
	UCHAR_MAX, // Breakable block direction
	UCHAR_MAX, // Breakable block Y bound extension
};

const u8 sChozodia_90_Clipdata[943] = {
    #include "extracted/data/rooms/chozodia/chozodia_90_clipdata.gfx.inc"
};

const u8 sChozodia_90_Bg1[1673] = {
    #include "extracted/data/rooms/chozodia/chozodia_90_bg1.gfx.inc"
};

const u8 sChozodia_90_Spriteset0[ENEMY_ROOM_DATA_ARRAY_SIZE(5)] = {
	10, 17, SPRITESET_IDX(0),
	10, 21, SPRITESET_IDX(0),
	16, 29, SPRITESET_IDX(0),
	28, 23, SPRITESET_IDX(2),
	ROOM_SPRITE_DATA_TERMINATOR
};

